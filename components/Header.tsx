...
export default function Header() {
  ...
  const cursosTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  ...
  const handleCursosMouseEnter = () => {
    if (cursosTimeoutRef.current) clearTimeout(cursosTimeoutRef.current)
    setCursosOpen(true)
  }

  const handleCursosMouseLeave = () => {
    cursosTimeoutRef.current = setTimeout(() => setCursosOpen(false), 150)
  }

  return (
    <>
      {/* Top Bar — Mobile: compact icons | Desktop: full text */}
      <div className="bg-brand-dark text-white/80 text-sm">
        {/* Mobile */}
        <div className="md:hidden container-main px-4 flex items-center justify-between py-2">
          ...
          <a href="https://wa.me/5531982948067?text=Olá, vim pelo site e quero saber mais sobre os serviços da Calazans Lumina" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-green-400 font-medium text-xs">
        ...
        <div className="hidden md:block">
          <div className="container-main flex items-center justify-between py-2.5">
            <div className="flex items-center gap-6">
              <a href="https://maps.app.goo.gl/umLNmLYMyhbVx4HQ6" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-brand-mint transition-colors">
            ...
            <a href="https://wa.me/5531982948067?text=Olá, vim pelo site e quero saber mais sobre os serviços da Calazans Lumina" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-brand-mint transition-colors font-medium">
      ...
      {/* Main Header */}
      <header className={`sticky top-0 z-50 transition-[transform,box-shadow,background-color,color] duration-300 bg-black ${scrolled ? 'shadow-lg shadow-black/30' : ''}`}>
        <div className="container-main flex items-center justify-between py-3">
          <Link href="/" className="flex items-center gap-3 group">
            {/* O arquivo antigo (CDN do GHL, 699cdc6d...) tinha 246px de vao
                entre o simbolo C e a palavra — 15% da largura da logo. Espaco
                entre letras da mesma palavra fica em 2 a 4%, entao o olho lia
                duas coisas: um icone e a palavra "alazans". Estava assim em
                todas as paginas.
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <Link href="/" className="text-white/70 hover:text-white hover:bg-white/10 transition-[transform,box-shadow,background-color,color] duration-200 text-sm font-medium px-4 py-2 rounded-lg">
              Home
            </Link>
            ...
            >
              <Link
                href="/servicos"
                className={`flex items-center gap-1 text-sm font-medium px-4 py-2 rounded-lg transition-[transform,box-shadow,background-color,color] duration-200 ${
              ...
              {/* Dropdown Panel */}
              <div
                className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[420px] bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden transition-[transform,box-shadow,background-color,color] duration-200 origin-top ${
            ...
            {/* Cursos Dropdown */}
            <div
              ref={cursosDropdownRef}
              className="relative"
              onMouseEnter={handleCursosMouseEnter}
              onMouseLeave={handleCursosMouseLeave}
            >
              <Link
                href="/cursos"
                className={`flex items-center gap-1 text-sm font-medium px-4 py-2 rounded-lg transition-[transform,box-shadow,background-color,color] duration-200 ${
                  cursosOpen ? 'text-white bg-white/10' : 'text-white/70 hover:text-white hover:bg-white/10'
                }`}
              >
                Cursos
                <svg className={`w-3.5 h-3.5 transition-transform duration-200 ${cursosOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </Link>
              ...
              <div
                className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[420px] bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden transition-[transform,box-shadow,background-color,color] duration-200 origin-top ${
                  cursosOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'
                }`}
              >
                <div className="p-3">
                  {cursosMenu.map((curso) => (
                    <Link
                      key={curso.slug}
                    ...
                    >
                      <div className="w-9 h-9 rounded-lg bg-brand-mint/10 flex items-center justify-center flex-shrink-0 group-hover/item:bg-brand-mint/20 transition-colors">
                        <svg className="w-4.5 h-4.5 text-brand-mint" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d={curso.icon} />
                        </svg>
                      </div>
                      <div>
                        <p className="text-white text-sm font-medium group-hover/item:text-brand-mint transition-colors">{curso.nome}</p>
                        <p className="text-white/40 text-xs mt-0.5">{curso.desc}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {navLinks.filter((l) => l.href !== '/').map((link) => (
              <Link key={link.href} href={link.href} className="text-white/70 hover:text-white hover:bg-white/10 transition-[transform,box-shadow,background-color,color] duration-200 text-sm font-medium px-4 py-2 rounded-lg">
                {link.label}
              </Link>
            ))}
          </nav>

          <a href="https://wa.me/5531982948067?text=Olá, vim pelo site e quero saber mais sobre os serviços da Calazans Lumina" target="_blank" rel="noopener noreferrer" className="hidden md:inline-flex items-center gap-2 bg-brand-mint hover:bg-brand-light text-white font-bold px-6 py-3 rounded-lg transition-[transform,box-shadow,background-color,color] duration-300 hover:scale-105 text-sm">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
            Fale Conosco
          </a>
        ...
        {menuOpen && (
          <div className="lg:hidden bg-black border-t border-white/10">
            <nav className="flex flex-col px-6 py-4 gap-1">
              ...
              >
                Cursos
                <svg className={`w-4 h-4 transition-transform duration-200 ${mobileCursosOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {mobileCursosOpen && (
              ...
              <a href="https://wa.me/5531982948067?text=Olá, vim pelo site e quero saber mais sobre os serviços da Calazans Lumina" target="_blank" rel="noopener noreferrer" className="bg-brand-mint text-white font-bold py-3 px-4 rounded-lg text-center mt-4">
                Fale pelo WhatsApp
              </a>
            </nav>
          </div>
