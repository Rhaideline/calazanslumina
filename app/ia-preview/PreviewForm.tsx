...
export default function PreviewForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/preview-request', {
        ...
        body: JSON.stringify({ nome, whatsapp, email, source: 'ia-preview-page' }),
      })
    ...
    return (
      <div className="bg-green-500/10 border-2 border-green-500/40 rounded-3xl p-8 text-center">
        ...
        <p className="text-white/70 text-sm leading-relaxed mb-5">
          O PDF começou a baixar automaticamente.
          Também enviei pro seu WhatsApp <strong className="text-white">{whatsapp}</strong>.
        </p>
        <p className="text-white/55 text-xs">
          Não baixou? <a href="/pdfs/ia-completo-preview.pdf" download className="text-red-400 underline">Clica aqui</a>.
        </p>
        <a
          href="/ia-completo"
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/[0.04] border border-white/10 rounded-3xl p-6 md:p-8 space-y-4">
      ...
      {status === 'error' && (
        <p className="text-red-400 text-sm text-center">Erro: {errorMsg}. Tenta novamente.</p>
      )}
    </form>
  )
}
